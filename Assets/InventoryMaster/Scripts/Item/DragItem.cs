...
public class DragItem : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
{
	...
	void Update(){
		temp=GameObject.FindGameObjectWithTag("Player").transform.localPosition;
	}
    ...
    public void OnEndDrag(PointerEventData data)
    {
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            else
            {
				...
				temp.x=temp.x+0.5f;
				dropItem.transform.localPosition =temp; //GameObject.FindGameObjectWithTag("Player").transform.localPosition;
                inventory.OnUpdateItemList();
